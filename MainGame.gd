...
func _input(event):
	if Input.is_action_just_pressed("ui_up") and not snake_direction == Vector2(0,1): 
		snake_direction = Vector2(0,-1)
	if Input.is_action_just_pressed("ui_right") and not snake_direction == Vector2(-1,0): 
		snake_direction = Vector2(1,0)
	if Input.is_action_just_pressed("ui_left") and not snake_direction == Vector2(1,0): 
		snake_direction = Vector2(-1,0)
	if Input.is_action_just_pressed("ui_down") and not snake_direction == Vector2(0,-1): 
		snake_direction = Vector2(0,1)
...
func check_apple_eaten():
	if apple_pos == snake_body[0]:
		apple_pos = place_apple()
		add_apple = true
		get_tree().call_group("ScoreGroup","update_score",snake_body.size() - 2)
		$CrunchSound.play()
